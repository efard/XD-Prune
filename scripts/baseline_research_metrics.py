...
def run_coco_evaluation(ground_truth: Path, predictions: Path, output_dir: Path) -> Path:
    ...
    excluded_non_positive_boxes: list[int] = []
    for index, prediction in enumerate(prediction_data):
        ...
        # Ultralytics exports predictions after clipping them to image bounds. A
        # prediction whose extent lies wholly beyond an edge can consequently
        # have zero width or height. It is not a valid COCO detection and has
        # no possible true-positive contribution, so exclude it only from this
        # secondary COCO calculation. Keep the original export untouched and
        # record every exclusion count below for auditability.
        if float(bbox[2]) <= 0 or float(bbox[3]) <= 0:
            excluded_non_positive_boxes.append(index)
            continue
        valid_predictions.append(prediction)
    if not valid_predictions:
        result = {
            "status": "NO_PREDICTIONS",
            "note": "No positive-area predictions were available for secondary COCO-style evaluation.",
            "predictions": str(predictions),
            "predictions_sha256": sha256(predictions),
            "prediction_records_source": len(prediction_data),
            "prediction_records_evaluated": 0,
            "excluded_non_positive_box_records": len(excluded_non_positive_boxes),
    ...
    with redirect_stdout(log):
        ...
        dt = gt.loadRes(valid_predictions)
    ...
    result = {
        ...
        "predictions_sha256": sha256(predictions),
        "prediction_records_source": len(prediction_data),
        "prediction_records_evaluated": len(valid_predictions),
        "excluded_non_positive_box_records": len(excluded_non_positive_boxes),
        "excluded_non_positive_box_policy": (
            "Excluded only from this secondary COCO calculation after Ultralytics image-bound clipping; "
            "the original full-precision prediction export is retained unchanged."
        ),
        "ground_truth": str(ground_truth),
        "ground_truth_sha256": sha256(ground_truth),
