...
Each YAML under `Pruning_Study/data_views/` is intentionally relative to its
...
```bash
python Pruning_Study/scripts/prepare_detection_datasets.py --dataset both \
  --mio-root /path/to/mio --acdc-root /path/to/acdc \
  --output-root Pruning_Study/data_views --seed 42
```
