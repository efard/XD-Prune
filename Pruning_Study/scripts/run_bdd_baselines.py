...
Examples (from the workspace root):
    python Pruning_Study/scripts/run_bdd_baselines.py --dataset gen2 --check-only
    python Pruning_Study/scripts/run_bdd_baselines.py --dataset gen2 --full
    python Pruning_Study/scripts/run_bdd_baselines.py --dataset ngn2 --full
    python Pruning_Study/scripts/run_bdd_baselines.py --dataset gen2 --finalize-existing
"""
...
def validate_baseline(baseline: BddBaseline) -> dict[str, Any]:
    ...
    if config.get("project") != "Pruning_Study/results/baselines" or config.get("exist_ok") is not False:
