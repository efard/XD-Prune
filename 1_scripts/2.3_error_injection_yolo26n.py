...
For each selected layer, the script adds Gaussian noise to that layer's output activation
and evaluates mAP50-95 on the validation split. A larger mAP drop means the layer is more
sensitive and should be modified carefully.

Default behavior:
    - uses split=val
    - excludes Detect layers by default because the goal is to identify internal layers for
      pruning / quantization / possible acceleration decisions
    - never uses the test split unless the user explicitly passes --split test
"""
...
for layer_id in target_layer_ids:
    ...
    def inject_noise(_module, _inputs, output):
        # The hook changes only the selected layer output during this validation run.
