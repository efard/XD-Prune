...
class DynamicNoParamLayerAdapter(nn.Module):
    ...
    resizes spatial dimensions with nearest-neighbour interpolation,
    and crops or zero-pads channels to match the original layer's output interface.
    """
    ...
    def _dynamic_target_hw(self, reference: torch.Tensor) -> tuple[int, int]:
        # PNNX exports through torch.jit.trace(). During tracing,
        # shape values can be represented as scalar tensors,
        # while Python round() accepts only ordinary Python numeric values. Convert the dimensions first.
        reference_height = int(reference.shape[-2])
        reference_width = int(reference.shape[-1])

        # Keep the original rounding and minimum-size behaviour unchanged.
        height = max(1, round(reference_height * self.spatial_h_num / self.spatial_h_den),)
        width = max(1, round(reference_width * self.spatial_w_num / self.spatial_w_den),)
        return height, width
