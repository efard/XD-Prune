...
# 0 = process the full video.
MAX_FRAMES = 0
JPEG_QUALITY = 85

r'''
Pynq ssh:
bash 1_scripts/run_live_server.sh 4_layer_replacement_p56_320_int8
...
def main() -> None:
    ...
    # Each command-line argument uses the editable value at the top of this
    # script as its default. Therefore:
    #   1. Running without arguments uses the script configuration.
    #   2. Supplying an argument overrides only that specific value.
    parser.add_argument(
        "--host",
        default=HOST,
        help="PYNQ IP address. Default: %(default)s",
    )
    parser.add_argument(
        "--video",
        default=VIDEO,
        help="Path to the video file on this PC. Default: %(default)s",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=PORT,
        help="TCP server port. Default: %(default)s",
    )
    parser.add_argument(
        "--jpeg-quality",
        type=int,
        default=JPEG_QUALITY,
        help="JPEG quality from 1 to 100. Default: %(default)s",
    )
    parser.add_argument(
        "--send-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=MAX_FRAMES,
        help="0 means process the full video; use 3 first for a smoke test. Default: %(default)s",
    )
    parser.add_argument(
        "--display-width",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "2 = process every 3rd frame, etc. Default: %(default)s"
        ),
    )
    args = parser.parse_args()

    # Print the effective configuration so it is immediately clear whether the
    # values came from the script defaults or command-line overrides.
    print("===== Effective configuration =====")
    print("HOST: {}".format(args.host))
    print("VIDEO: {}".format(args.video))
    print("PORT: {}".format(args.port))
    print("JPEG_QUALITY: {}".format(args.jpeg_quality))
    print("SEND_SIZE: {}".format(args.send_size))
    print("MAX_FRAMES: {}".format(args.max_frames))
    print("DISPLAY_WIDTH: {}".format(args.display_width))
    print("FRAME_SKIP: {}".format(args.frame_skip))
    print("===================================")
