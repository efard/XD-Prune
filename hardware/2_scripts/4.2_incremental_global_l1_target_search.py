...
- 54 protected roots may receive required input-channel updates, but their output channels must never be removed.

Each eligible root retain at least:
    max(4 channels, ceil(50% of its original output channels))
This prevents pure Global L1 from collapsing almost an entire layer while remaining a global L1-magnitude method.

This search saves CSV/JSON plans only.
"""
